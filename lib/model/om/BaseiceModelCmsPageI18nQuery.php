...
abstract class BaseiceModelCmsPageI18nQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = iceModelCmsPageI18nPeer::getInstanceFromPool(serialize(array((string) $key[0], (string) $key[1]))))) && !$this->formatter)
    {
   ...
   *
   * @return    iceModelCmsPageI18n A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `NAME`, `SLUG`, `CONTENTS`, `META_DESCRIPTION`, `META_KEYWORDS`, `CULTURE` FROM `cms_page_i18n` WHERE `ID` = :p0 AND `CULTURE` = :p1';
    try
    {
    ...
    {
      $obj = new iceModelCmsPageI18n();
      $obj->hydrate($row);
      iceModelCmsPageI18nPeer::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1])));
    }
    $stmt->closeCursor();
