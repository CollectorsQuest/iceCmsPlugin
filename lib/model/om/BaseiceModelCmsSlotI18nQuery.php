...
abstract class BaseiceModelCmsSlotI18nQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = iceModelCmsSlotI18nPeer::getInstanceFromPool(serialize(array((string) $key[0], (string) $key[1]))))) && !$this->formatter)
    {
   ...
   *
   * @return    iceModelCmsSlotI18n A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `CONTENTS`, `CULTURE` FROM `cms_slot_i18n` WHERE `ID` = :p0 AND `CULTURE` = :p1';
    try
    {
    ...
    {
      $obj = new iceModelCmsSlotI18n();
      $obj->hydrate($row);
      iceModelCmsSlotI18nPeer::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1])));
    }
    $stmt->closeCursor();
