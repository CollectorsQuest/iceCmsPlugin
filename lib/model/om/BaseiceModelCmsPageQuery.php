...
abstract class BaseiceModelCmsPageQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = iceModelCmsPagePeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    iceModelCmsPage A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `TREE_LEFT`, `TREE_RIGHT`, `TREE_SCOPE`, `IS_PUBLISHED`, `UPDATED_AT`, `CREATED_AT` FROM `cms_page` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new iceModelCmsPage();
      $obj->hydrate($row);
      iceModelCmsPagePeer::addInstanceToPool($obj, (string) $key);
    }
    $stmt->closeCursor();
