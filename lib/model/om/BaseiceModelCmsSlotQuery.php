...
abstract class BaseiceModelCmsSlotQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = iceModelCmsSlotPeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    iceModelCmsSlot A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `TYPE`, `NAME`, `UPDATED_AT`, `CREATED_AT` FROM `cms_slot` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new iceModelCmsSlot();
      $obj->hydrate($row);
      iceModelCmsSlotPeer::addInstanceToPool($obj, (string) $key);
    }
    $stmt->closeCursor();
